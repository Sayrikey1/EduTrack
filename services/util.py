...
class CustomApiRequestProcessorBase(
    ...
    paginate_results = False  # Enable/disable pagination
    ...
    #     # Decrypt request data if encryption is enabled
    #     if self.request_payload_requires_decryption or settings.APP_ENC_ENABLED:
    #         encryption_util = AESCipher(settings.APP_ENC_KEY, settings.APP_ENC_VEC)
    #         request_data = encryption_util.decrypt_body(request.data)
    #     else:
    ...
    #     # Prepare context for serializers
    #     if not self.context:
    #         self.context = dict()
    #     self.context["request"] = request
    #     if self.extra_context_data:
    #         self.context.update(self.extra_context_data)
    
    #     try:
    #         # Validate input using serializer (for POST/PUT/PATCH)
    #         if self.serializer_class and request.method in {"POST", "PUT", "PATCH", "DELETE", "GET"}:
    #             # Handle GET requests separately (path/query params)
    #             if request.method == "GET":
    #                 path_params = request.resolver_match.kwargs if hasattr(request, "resolver_match") else {}
    #                 query_params = request.query_params.dict()
    #                 serializer_data = {**path_params, **query_params}
    #             else:
    #                 serializer_data = request_data or dict()
    ...
    #         # Call the target function (e.g., service.list_courses)
    #         response_raw_data: Union[tuple, T] = target_function(validated_data, **extra_args)
    
    #         # Handle errors
    #         if isinstance(response_raw_data, tuple) and response_raw_data[1]:
    #             error_data = response_raw_data[1]
    #             status_code = error_data.status_code if isinstance(error_data, OperationError) else None
    #             return self.response_with_error(error_data, status_code=status_code)
    
    #         # Extract the queryset or data
    #         queryset_or_data = response_raw_data[0] if isinstance(response_raw_data, tuple) else response_raw_data
    
    #         # Apply pagination if enabled
    #         if self.paginate_results and isinstance(queryset_or_data, QuerySet):
    #             paginator = self.pagination_class()
    #             page = paginator.paginate_queryset(queryset_or_data, request)
    ...
    #             response_raw_data = (paginated_response.data, None)
    ...
    #         AppLogger.report(e)
    #         return self.response_with_json(
    #             {"error": str(e), "message": "Server error"},
    #             status_code=status.HTTP_500_INTERNAL_SERVER_ERROR
    #         )
