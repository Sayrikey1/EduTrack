...
class LoginSerializer(TokenObtainSerializer):
    ...
    username = serializers.CharField(
        ...
        help_text="Enter your username OR your email address",
        trim_whitespace=True,
        error_messages={
            "blank": "Please provide your username or email.",
            "required": "Please provide your username or email.",
        },
    )
        ...
        try:
            ...
            raise serializers.ValidationError(
                render_template_to_text(
                    translate("auth.login.error"), {"error": str(e)}
                ),
                "username",
            )
    ...
    def validate(self, attrs):
        ...
        if not password_schema.validate(password):
            raise serializers.ValidationError(
                ...
                "Your password should contain at least 8 characters, an uppercase, a lowercase, a digit and symbol ",
                "password",
            )
        ...
        if User.objects.filter(Q(email__iexact=email)).exists():
            ...
            raise serializers.ValidationError(
                "An account with the provided email already exists", "email"
            )
