...
from accounts.tasks import (
    send_activation_otp_email_queue,
...
class AuthService(CustomAPIRequestUtil):
    def login(self, payload) -> dict:
        ...
        user_type = user.user_type or UserTypes.student
        ...
        try:
            ...
            if log:
                ...
                if not log.is_verified:
                    ...
                    # )
                    message = (
                        f"An OTP has already been sent. Please check your messages."
                    )
        ...
        try:
            ...
            if otp_expired:
                ...
                return None, self.make_error(
                    "Provided OTP has expired, please request for new OTP"
                )
    ...
    def register(self, payload, register_type):
        try:
            ...
            student, teacher, client_error = None, None, None
            payload["user_type"] = register_type
            if register_type == UserTypes.student:
                student, client_error = client_service.register(payload, user=user)
            if register_type == UserTypes.teacher:
                teacher, client_error = client_service.register(payload, user=user)
            ...
            payload["update_kyc_required"] = register_type == UserTypes.teacher
